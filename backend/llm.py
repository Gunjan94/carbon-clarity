...
Makes the board-summary layer genuinely AI-driven WITHOUT cloud credentials:
the narrative is produced by an actual language model at request time, not a
static template. Callers prefer, in order:
...
no API key). Non-streaming: we fetch once and the caller streams word-by-word.
Responses are cached on disk by hash of (model, system, user) for fast,
reproducible demos — same inputs reproduce the same words, different inputs
produce a genuinely different live generation. The output is always
model-generated, never hardcoded.
"""
...
def generate(system_prompt: str, user_prompt: str) -> str:
    ...
    for attempt in range(RETRIES + 1):
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "CarbonClarity/1.0"})
